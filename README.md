...
- **In-viewport sketching** — pick a plane (XY / XZ / YZ) from the command bar and the camera flattens square-on into an orthographic view. Draw with Line, Rect, Circle, Trim and Dimension tools; snapping keeps mouse-derived values on clean round numbers while anything you *type* is treated as exact and left untouched. Saving emits a tidy `new Sketch(...).LineTo(...).End(true).Face()` block into the editor as a single undoable edit.
